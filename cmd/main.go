...
func main() {
	search := "Zakhar Asli"
	res, err := GoogleScrape(search, "com", "en", 1, 30, nil)
	fmt.Printf("Data recived\n")
	if err == nil {
		WriteDataToJsonFile(search, res)
	}
}

func WriteDataToJsonFile(search string, data []models.SearchResult) {
	_, err := os.Stat("../recieved")
	if err != nil {
		err = os.Mkdir("recieved", os.ModeAppend)
		if err != nil {
			fmt.Printf("Dir not exists")
			return
		}
	}
	fileName := "./recieved/" + search + ".json"
	...
	ioutil.WriteFile(fileName, jsonData, os.ModeAppend)
}
...
func GoogleScrape(search, countryCode, languageCode string, pages, count int, proxyString interface{}) ([]models.SearchResult, error) {
	results := []models.SearchResult{}
	resultsConuter := 0
	googlePages, err := BuildGoogleURLS(search, countryCode, languageCode, pages, count)
	if err != nil {
		return nil, err
	}
	for _, page := range googlePages {
		...
		data, err := GoogleResultParsing(res, resultsConuter)
		if err != nil {
			return nil, err
		}
		resultsConuter += len(data)
...
func BuildGoogleURLS(search, countryCode, languageCode string, pages, count int) ([]string, error) {
	toScrape := []string{}
	searchTerm := strings.Trim(search, " ")
	searchTerm = strings.Replace(searchTerm, " ", "+", -1)
	googleBase, found := googleDomains[countryCode]
	if found {
		for i := 0; i < pages; i++ {
			start := i * count
...
func GoogleResultParsing(res *http.Response, rank int) ([]models.SearchResult, error) {
	...
	for i := range sel.Nodes {
		...
		if link != "" && link != "#" && !strings.HasPrefix(link, "/") {
			result := models.SearchResult{ResultTitle: title, ResultRank: rank, ResultURL: link, ResultDesc: desc}
			results = append(results, result)
		}
		rank++
	}
